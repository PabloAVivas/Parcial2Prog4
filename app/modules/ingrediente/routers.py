...
def alta_ingrediente(admin: Annotated[UsuarioRead, Depends(require_role(["ADMIN"]))], ingrediente: IngredienteCreate, session: SeDe) -> IngredienteRead:
...
def listar_ingredientes(admin: Annotated[UsuarioRead, Depends(require_role(["ADMIN"]))],offset: int = Query(0, ge=0), limit: int = Query(20, ge=1, le=100), nombre: str = Query(default=None) , session: IngredienteService = Depends(get_ingrediente_service)) -> IngredientePaginadoResponse:
...
def detalle_ingrediente(admin: Annotated[UsuarioRead, Depends(require_role(["ADMIN"]))],session: SeDe, ingrediente_id: int = Path(gt=0) ) -> IngredienteRead:
...
def actualizar_ingrediente(admin: Annotated[UsuarioRead, Depends(require_role(["ADMIN"]))], datos: IngredienteUpdate, session: SeDe, ingrediente_id: int = Path(gt=0) ) -> IngredienteRead:
...
def eliminar_ingrediente(admin: Annotated[UsuarioRead, Depends(require_role(["ADMIN"]))], session: SeDe, ingrediente_id: int = Path(gt=0) ) -> None:
