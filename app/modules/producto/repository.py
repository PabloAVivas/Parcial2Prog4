...
class ProductoRepository(BaseRepository[Producto]):
    ...
    def get_activo(self, offset: int = 0, limit: int = 100, nombre: str = None, categoria_id: int | None = None, disponible: bool | None = None) -> list[Producto]:
        ...
        if categoria_id is not None:
            query = query.where(
                exists(select(ProductoCategoriaLink.producto_id).where(
    ...
    def count_activo(self, nombre: str = None, categoria_id: int | None = None, disponible: bool | None = None) -> int:
        query = select(func.count()).select_from(Producto).where(Producto.activo == True)
        if nombre:
            query = query.where(Producto.nombre.ilike(f"%{nombre}%"))
        if categoria_id is not None:
